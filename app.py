...
def homepage():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            new_transaction = pd.DataFrame([[username, amount, description, transaction_date]], 
                                           columns=['Username', 'Amount', 'Description', 'Date'])
            
            # Write the new transaction with the current date to the Transactions sheet
            with pd.ExcelWriter(DATA_FILE, mode='a', if_sheet_exists='overlay') as writer:
